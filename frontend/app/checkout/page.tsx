...
export default function CheckoutPage() {
  ...
  const deliveryFee = subtotal >= 500 ? 0 : 25; const total = subtotal + deliveryFee;
  ...
  return <section className="checkout-page"><form onSubmit={submit} className="checkout-form"><Link href="/cart" className="back-link">← Back to bag</Link><p className="eyebrow">Secure checkout</p><h1>Delivery details</h1>
    ...
    <div className="form-section"><h2>Delivery method</h2><label className={delivery === 'accra' ? 'radio-option selected' : 'radio-option'}><input type="radio" name="delivery" value="accra" checked={delivery === 'accra'} onChange={() => setDelivery('accra')} /><span><b>Accra delivery</b><small>1–2 working days</small></span><strong>{subtotal >= 500 ? 'Free' : formatGHS(25)}</strong></label><label className={delivery === 'standard' ? 'radio-option selected' : 'radio-option'}><input type="radio" name="delivery" value="standard" checked={delivery === 'standard'} onChange={() => setDelivery('standard')} /><span><b>Ghana standard delivery</b><small>2–5 working days</small></span><strong>{subtotal >= 500 ? 'Free' : formatGHS(25)}</strong></label></div>
    <div className="payment-note"><b>Payment comes next</b><p>{process.env.NEXT_PUBLIC_CHECKOUT_MODE === 'api' ? 'You will continue to secure Paystack checkout for Mobile Money or card.' : 'This demo creates a test order without charging you.'}</p></div>{error && <p className="form-error">{error}</p>}<button disabled={busy} className="button primary full">{busy ? 'Preparing your order…' : `Continue to payment · ${formatGHS(total)}`}</button><p className="fine-print">By continuing, you agree to our <Link href="/policies/terms">terms</Link> and <Link href="/policies/privacy">privacy policy</Link>.</p>
