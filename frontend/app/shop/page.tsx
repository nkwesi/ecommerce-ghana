...
export default async function ShopPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  return <div className="shop-page section">
    ...
    <div className="shop-toolbar"><div className="filters"><Link className={!category ? 'active' : ''} href="/shop">All</Link>{['Women', 'Men', 'Essentials'].map((item) => <Link className={category === item ? 'active' : ''} href={`/shop?category=${item}`} key={item}>{item}</Link>)}</div><span>{filtered.length} pieces</span></div>
